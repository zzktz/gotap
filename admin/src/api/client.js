...
client.interceptors.request.use((config) => {
  const token = localStorage.getItem('gotap_admin_token') || localStorage.getItem('goyou_admin_token') || localStorage.getItem('proxyswitch_admin_token')
...
client.interceptors.response.use(
  ...
  (error) => {
    ...
    if (status === 401 && !isAdminLoginRequest && !authExpiredRedirectTimer) {
      localStorage.removeItem('gotap_admin_token')
