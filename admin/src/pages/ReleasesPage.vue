...
async function load(page = pagination.current, pageSize = pagination.pageSize, showSpinner = true) {
  ...
  catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '版本列表加载失败') }
...
async function createRelease() {
  ...
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '创建版本失败') }
...
function importLatestRelease() {
  Modal.confirm({ title: '一键创建版本', content: '将从 GitHub 最新 Release 下载三平台更新包和签名，创建为后台草稿。确认继续吗？', okText: '开始创建', cancelText: '取消', async onOk() {
    ...
    } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '一键创建失败'); throw error }
...
async function saveNotes() {
  ...
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '保存更新说明失败') }
  finally { saving.value = false }
}
async function uploadAsset(record, platform) {
  ...
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '更新包上传失败') }
...
async function publish(record) {
  ...
  catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '版本发布失败') }
}
async function unpublish(record) {
  ...
  catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '版本撤回失败') }
}
function deleteRelease(record) {
  Modal.confirm({ title: '删除版本', content: `确定删除 v${record.version} 及其所有更新文件吗？删除后不可恢复。`, okText: '删除', okType: 'danger', cancelText: '取消', async onOk() {
    try {
    ...
    catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '删除版本失败') }
  } })
}
onMounted(() => {
  resetFiles(); load()
  ...
  <div>
    ...
    <a-card :bordered="false"><a-spin :spinning="loading"><a-empty v-if="!rows.length && !loading" description="还没有版本草稿" /><a-collapse v-else accordion>
      <a-collapse-panel v-for="record in rows" :key="record.id">
        ...
        <div class="release-actions"><a-button v-if="record.status === 'draft'" @click="openEdit(record)"><EditOutlined />编辑说明</a-button><a-button v-if="record.status === 'draft'" type="primary" :disabled="record.download_status === 'downloading'" @click="publish(record)">发布版本</a-button><a-button v-else @click="unpublish(record)">撤回版本</a-button><a-button danger :disabled="record.download_status === 'downloading'" @click="deleteRelease(record)"><DeleteOutlined />删除版本</a-button></div>
      </a-collapse-panel>
