...
async function load() {
  ...
  try {
    ...
    if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '个人信息加载失败')
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    ...
    if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '个人信息保存失败')
  } finally {
    saving.value = false
  }
}
