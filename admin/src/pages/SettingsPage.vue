...
async function load() {
  ...
  try {
    ...
    if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '系统设置加载失败')
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    ...
    if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '系统设置保存失败')
    return false
  } finally {
    saving.value = false
  }
