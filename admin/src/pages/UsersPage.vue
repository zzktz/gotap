...
async function load(page = pagination.current, pageSize = pagination.pageSize) {
  ...
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '用户列表加载失败') } finally { loading.value = false }
}
...
async function changeStatus(record, enabled) {
  ...
  try { await client.patch(`/v1/admin/users/${record.id}/status`, { enabled }); record.enabled = enabled; message.success(enabled ? '用户已启用' : '用户已停用') } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '用户状态更新失败') } finally { updatingId.value = '' }
}
...
async function saveCreate() {
  ...
  try {
    ...
    await load(1)
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '用户创建失败') } finally { createSaving.value = false }
}
async function saveExpiry() {
  ...
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '到期时间更新失败') } finally { expirySaving.value = false }
}
async function saveQuota() {
  ...
  } catch (error) { if (!error.adminAuthExpired) message.error(error.response?.data?.detail || '流量额度更新失败') } finally { quotaSaving.value = false }
}
onMounted(load)
</script>
...
<template>
  <div>
    <div class="page-title"><div><h1>用户管理</h1><p>查看注册用户、账户状态和使用额度。</p></div><a-space><a-button @click="load()">刷新</a-button><a-button type="primary" @click="openCreate">添加用户</a-button></a-space></div>
    ...
    <a-modal v-model:open="quotaOpen" title="设置每日额度" :confirm-loading="quotaSaving" ok-text="保存" cancel-text="取消" @ok="saveQuota"><p v-if="quotaUser">用户：{{ quotaUser.email }}</p><a-form-item label="每日额度"><a-input-number v-model:value="quotaMegabytes" :min="0" :max="10000000" :precision="0" addon-after="MB" style="width: 100%" /></a-form-item><p class="muted">当前已使用：{{ formatBytes(quotaUser?.used_bytes) }}。</p></a-modal>
    <a-modal v-model:open="expiryOpen" title="设置账户到期时间" :confirm-loading="expirySaving" ok-text="保存" cancel-text="取消" @ok="saveExpiry"><p v-if="expiryUser">用户：{{ expiryUser.email }}</p><a-form-item label="到期日期"><a-input v-model:value="expiryDate" type="date" /></a-form-item><p class="muted">留空表示长期有效。</p></a-modal>
